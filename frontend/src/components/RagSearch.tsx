    ...
    <div className="space-y-6">
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className={`rounded-xl border p-4 ${health?.ollamaAvailable ? 'bg-green-50 border-green-200' : 'bg-red-50 border-red-200'}`}>
          ...
          <p className={`text-lg font-bold ${health?.ollamaAvailable ? 'text-green-700' : 'text-red-700'}`}>
            {health === null ? '...' : health.ollamaAvailable ? t('ragSearch.available') : t('ragSearch.unavailable')}
          </p>
        </div>
        <div className="rounded-xl border p-4 bg-blue-50 border-blue-200">
          <p className="text-xs font-medium text-blue-600">{t('ragSearch.processedEmails')}</p>
          <p className="text-lg font-bold text-blue-700">{stats?.embeddedChunks?.toLocaleString(locale) ?? '-'}</p>
        </div>
        <div className="rounded-xl border p-4 bg-amber-50 border-amber-200">
          <p className="text-xs font-medium text-amber-600">{t('dashboard.pendingChunks')}</p>
          <p className="text-lg font-bold text-amber-700">{stats?.pendingChunks?.toLocaleString(locale) ?? '-'}</p>
        </div>
        <div className="rounded-xl border p-4 bg-gray-50 border-gray-200">
          <p className="text-xs font-medium text-gray-600">{t('dashboard.totalEmails')}</p>
      ...
      {(mode === 'search' || mode === 'all') && (
        <div className="bg-blue-50 border border-blue-200 rounded-xl p-5">
          <h3 className="font-semibold text-blue-800 mb-2">{t('ragSearch.semanticSearchMoved')}</h3>
          <p className="text-sm text-blue-700 mb-3">
            {t('ragSearch.semanticSearchMovedDescription')}
          </p>
          <div className="flex flex-wrap gap-3">
            ...
            <a href="/knowledge-graph" className="inline-flex items-center gap-2 px-4 py-2 bg-white border border-blue-300 text-blue-700 text-sm font-medium rounded-lg hover:bg-blue-50 transition-colors">
              🕸 {t('ragSearch.kgSemanticAnalysis')}
            </a>
          </div>
        </div>
      )}
      ...
      {(mode === 'manage' || mode === 'all') && (
        <div className="bg-amber-50 border border-amber-200 rounded-xl p-5">
          <h3 className="font-semibold text-amber-800 mb-2">{t('ragSearch.indexManagementMoved')}</h3>
          <p className="text-sm text-amber-700 mb-3">
            {t('ragSearch.indexManagementMovedDescription')}
          </p>
          <a href="/knowledge-graph" className="inline-flex items-center gap-2 px-4 py-2 bg-white border border-amber-300 text-amber-700 text-sm font-medium rounded-lg hover:bg-amber-50 transition-colors">
            🕸 {t('ragSearch.openKnowledgeGraphPage')}
          </a>
        </div>
      )}
    </div>
