...
class ActivityGuard:
    ...
    def __init__(self, api: SynologyAPI):
        ...
        # For pst-extractor, we'll use different limits for embedding
        self.max_cpu_embed = float(os.getenv("MAX_NAS_CPU_EMBED", "40"))
        self.max_tx_embed = float(os.getenv("MAX_NAS_TX_MBPS_EMBED", "30"))
    ...
    def is_nas_idle_for_embedding(self) -> tuple[bool, str]:
        """Check if NAS is idle for embedding operations (stricter limits)."""
        cpu, tx = self.api.get_cpu_and_network()
        if cpu > self.max_cpu_embed:
            return False, f"NAS CPU magas (embedding): {cpu:.1f}% (limit: {self.max_cpu_embed}%)"
        if tx > self.max_tx_embed:
            return False, f"NAS hálózat magas (embedding): {tx:.2f} MB/s (limit: {self.max_tx_embed} MB/s)"
        return True, f"NAS OK – CPU: {cpu:.1f}%, TX: {tx:.2f} MB/s"
